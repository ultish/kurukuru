...
Read `.keel/prd/$ARGUMENTS.md`. Propose a set of **vertical** slices — each one
...
1. `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/keel.py" new-slice "<title>" --epic $ARGUMENTS`
...
4. `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/keel.py" set-status <id> ready --note "contract frozen"`

Finish with `keel ls` and tell the user to run `/keel:build`.
